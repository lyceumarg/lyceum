export function solicitudInscripcionHtml({
  alumnoNombre, alumnoEmail, curso, academia,
}: { alumnoNombre: string; alumnoEmail: string; curso: string; academia: string }) {
  return `<!DOCTYPE html>
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background-color:#ffffff;border:1px solid #dcdee8;border-radius:14px;overflow:hidden;">
        <tr><td style="background-color:#15171e;padding:24px 32px;">
          <span style="font-size:18px;font-weight:bold;color:#ffffff;">Lyceum</span>
        </td></tr>
        <tr><td style="padding:32px 32px 8px;">
          ...
          <p style="margin:0 0 24px;font-size:14.5px;line-height:1.6;color:#43465a;">
            ...
            inscribilo desde el panel: <strong>Cursos → el curso → pestaña "Inscripción masiva"</strong> — solo
            pegá su email (<span style="font-family:'Courier New',monospace">${alumnoEmail}</span>) y queda inscripto al instante.
          </p>
        </td></tr>
